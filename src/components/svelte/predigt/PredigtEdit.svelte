<script>
  ...
  onMount(() => {
    axios
      .get(
        "https://www.evang9.wien/root/wp-json/combo/v1/combotermine?from_date=-50&to_date=0"
      )
      .then((response) => {
    ...
    axios
      .get("https://www.evang9.wien/root/wp-json/combo/v2/ispredigtedit",getAuthHeader())
      .then((response) => {
        predigtEdit = true;
      });
  ...
  function loadPredigten() {
    axios
      .get("https://www.evang9.wien/root/wp-json/combo/v1/getpredigten")
      .then((response) => {
        predigten = Object.values(JSON.parse(response.data)).sort((a, b) =>
          a < b ? 1 : -1
        );
  ...
  function getName(termin) {    
    const name = termine.filter(
      (f) => f.Termin == termin && f.Veranstaltung == "GD"
    );
  ...
  {#if termine}
    {#if predigtEdit}
      <form id="predigtform" on:submit|preventDefault={submitForm}>
        <Card>
          <Content>
            Administrator-Modus: Bitte die Predigt als mp3 Datei und den Temin auswählen!
          </Content>
          <Actions>
            <ActionButtons>
    ...
    {/if}    
    {#if predigten}
      <DataTable table$aria-label="Liederliste">
        <Head>
          <Row>
            <Cell style="width: 60px;" />
            <Cell sylte="width: 30%">Termin<br />Name</Cell>
            <Cell>Predigt</Cell>
          </Row>
        </Head>
        <Body>
            ...
            <Row>
              <Cell style="text-align: center;padding: 6px">
                <img
                  class="prediger_img"
                  alt="prediger"
                  src="https://evang9.wien/comboapps/img/{getImg(predigt.split('_')[0])}"
                />
              </Cell>
              <Cell>
