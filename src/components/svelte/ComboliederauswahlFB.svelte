<script>
  ...
  const firebaseConfig = {
    ...
    appId: '1:110813316877:web:f5d52bbcd48b942b189244',
    measurementId: 'G-0EW96D5XX2',
  };
  ...
  onMount(async () => {
    if (fireBase) {
      ...
      const url = await getDownloadURL(ref(storage, 'lieder/mp3/above_all.mp3'));
    }
                ...
                <TableBodyRow>
                  ...
                  <TableBodyCell class="w-4">
                    <div class="flex flex-row">
                      {#if fireBase}
                        {#await getDownloadURL(ref(storage, 'lieder/noten/' + lied.Dateiname + '.pdf'))}
                          <p>loading</p>
                        {:then url}
                  ...
                  <TableBodyCell>
                    {#if lied.MP3 != '0'}
                      <div class="flex flex-row">
                        {#if fireBase}
                          {#await getDownloadURL(ref(storage, 'lieder/mp3/' + lied.Dateiname + '.mp3'))}
                            <p>loading</p>
                          {:then url}
